...
```
Запустится симуляция и с управлением с помощью клавиатуры.
...
Ниже пара показательных примеров. В промте была просьба подробно расписать возможные последствия выбора каждой из команд и на основе этих рассуждений принять решение. Подобные объяснения не сильно помогают понять причину неудачи. На первом кадре модель почему-то решила, что ехать прямо - значит удаляться от обхекта:
...
## Настройка робота
### OC
Настройка самого робота будет зависеть от конкретной модели. Например, для waveshare ровера нужно вручную отключить запускаемый при старте python-процесс, блокирующий камеру ([официальная инструция](https://www.waveshare.com/wiki/30_OpenCV_Color_Tracking#Terminate_Main_Demo)).

Настройка мини пк RaspberryPi начинается с установки на нее [Raspberry Pi OS](https://www.raspberrypi.com/software/operating-systems/). Я ставил bookworm версию, на всякий случай сохранил образ [ТУТ](https://disk.yandex.ru/d/wgu7vjcfcwhprQ). 
