...
## Введение
Во второй части — практическая больше практическая сторона проекта: установка и первый запуск, эксперименты по созданию идеального промпта (он теперь не содержит жестких инструкций поведения и работает быстрее), а также попытки добавить «память» о прошлом шаге.  В конце - переход от симуляции к реальному роверу: сборка, настройка и проверка работы на железе.

[Обещанный код проекта](https://github.com/andrybin/camerabot)
...
Теперь настало время заменить ручное управление на команды визуально-языковой модели (VLM). Сперва нужно установить [**ollama**](https://ollama.com/download) и скачать нужную модель: `ollama run qwen2.5vl:latest`. В последующие разы скачанная модель будет запускаться автоматически при старте автопилота. Как было сказано в первой части, оптимальным выбором стал **qwen2.5vl:latest** (7b), он стоит в launch-файлах по умолчанию.
 ...
 Пробовал запускать на двух ноутбуках, и вот какой неприятный момент обнаружил: qwen2.5vl, запущенный через ollama на ноутбучной RTX4080 комфортно занимает 8 из 12 Гб, а на A3000 (тоже 12 Гб, но чуть более старой архитектуры Ampere) не помещается полностью и пятую часть весов оставляет на цпу! Скорость при этом сразу падает в разы.
...
### Камера
[Широкоугольная камера](https://www.waveshare.com/ov9281-160-camera.htm) имеет угол обзора в 126 град. и это то что нужно, меньше не берите. Для таких камер часто надо докупать [шлейф](https://ozon.ru/t/39Rgc1N), потому что в 5-ой версии RaspberryPi уменьшили ширину гнезда для шлейфа камеры (зато их стало два). 
...
## Настройка робота
### OC
Настройка самого робота будет зависеть от конкретной модели. Например, для waveshare ровера сначала нужно вручную отключить запускаемый при старте python-процесс, блокирующий камеру ([официальная инструкция](https://www.waveshare.com/wiki/30_OpenCV_Color_Tracking#Terminate_Main_Demo)).
