...
## Введение
Во второй части — практическая часть проекта: установка и первый запуск, эксперименты с форматом ответов и промптами для более устойчивого поведения модели, а также попытки добавить «память» о прошлом шаге. В конце переходим от симуляции к реальному роверу: сборка, настройка и проверка работы на железе.
...
За отправку запросов к ollama и обработку её сообщений отвечает нода vlm_control. В файле **resourses/vlm_prompt.txt** записана управляющая инструкция или промпт, специально подобранный для 7b модели. Он загружается при старте ноды и его можно легко редактировать. Чтобы показать, почему итоговый вариант промпта именно такой, мы рассмотрим еще несколько его вариантов, постепенно добавляя необходимые ингридиенты.
...
Your goal:
Using available commands to correct your direction get close to WHITE BALL as mutch as possible and touch it.

What available command to choise to achive the goal? Answer in short!
```
...
### Цепочка рассуждений
Если позволить модели немного «подумать», можно получить более осознанное поведение. Нужно просто убрать требование отвечать коротко, модель по дефолту уже обучена размышлять в процессе генерации ответа:
...
What available command to choise to achive the goal?
```
...
- Answer in new line started with >>>: What available command to choise to achive the goal?
```
...
Чтобы выделить в ответе модели описание сцены на прошлом шаге простой эвристики недостаточно. К счастью можно просто попросить модель сделать это самой в рамках её же ответа. В промпте для этого есть такой спецсимвол «???»:
`
Кот
`
Краткое описание предыдущей сцены так же подается в начало промпта со специальной пометкой. Любопытно, но эффект еще хуже чем при добавлении последней команды, робот просто не может думать:
